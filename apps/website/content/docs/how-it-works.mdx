...
- **CSS styles**\
Available in both `@prose-ui/react/prose-ui.css` and `@prose-ui/next/prose-ui.css` (plus their matching `katex.min.css` entries).
- **Remark plugins**\
...
CSS styles from `@prose-ui/react/prose-ui.css` (or the matching path under `@prose-ui/next`) provide default styles for typography and Prose UI components, scoped under the `prose-ui` ancestor CSS class.
...
If you're using the Next.js-specific entry point, you can keep the previous imports:
...
The easiest way to customize styles is by overriding the CSS variables defined in [`@prose-ui/react/prose-ui.css`](https://github.com/vrepsys/prose-ui/blob/main/packages/styles/dist/prose-ui.css).
</Callout>
